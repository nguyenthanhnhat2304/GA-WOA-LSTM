...
for typename in ["list", "dict", "set", "sum", "input", "print"]:
    if not isinstance(getattr(builtins, typename), type(eval(typename))):
        raise TypeError(
            f"Loại tích hợp '{typename}' bị ô nhiễm, vui lòng kiểm tra xem bạn đã sử dụng `{typename} = ...` ở đâu đó chưa"
        )

print("Kiểm tra môi trường thành công, bắt đầu chạy chương trình chính...")
...
def evaluate_lstm(individual, X_train, y_train):
    ...
    y_val_pred_inv = (
        y_val_pred * (scaler.data_max_[-1] - scaler.data_min_[-1])
        + scaler.data_min_[-1]
    )
    y_val_inv = (
...
def initialize_population(size):
    return [
        [random.uniform(*param_bounds[k]) for k in param_bounds] for _ in range(size)
    ]


def adaptive_k_range(generation, max_gen, base_range=(2, 4), final_range=(4, 5)):
    """
    Khoảng k được chọn trong giải đấu được điều chỉnh linh hoạt khi quá trình lặp lại diễn ra, chuyển từ phương pháp thăm dò sang phương pháp hội tụ.
    """
...
def mutation(ind, generation, max_gen):
    rate = 0.5 - 0.4 * (generation / max_gen)  # Giảm dần: 0.5 -> 0.1
    return [
        (
            val + np.random.normal(0, rate * (param_bounds[k][1] - param_bounds[k][0]))
...
def woa_update(pop, best, a):
    ...
    for ind in pop:
        ...
        if p < 0.33:
            ...
            new_ind = [
                d * np.exp(1 * l) * np.cos(2 * np.pi * l) + b for d, b in zip(D, best)
            ]
        else:
...
def ga_woa_optimize(X_train, y_train, pop_size=20, generations=15):
    ...
    for g in range(generations):
        ...
        next_gen[0] = best.copy()  # Bảo tồn tinh hoa
        a = 2 - g * (2 / generations)
...
print("\nKết quả đánh giá tập dữ liệu huấn luyện:")
...
print("\nKết quả đánh giá tập dữ liệu kiểm tra:")
...
pd.DataFrame({"Actual_Close": y_test_inv, "Predicted_Close": y_pred_inv}).to_csv(
    "train_results/GA_WOA_LSTM_test_prediction.csv", index=False
)
