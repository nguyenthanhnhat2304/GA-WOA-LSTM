...
class SlidingWindowDataLoaderSimple:
    def __init__(
        ...
        # Default save directory is data/processed/ relative to this file
        self.save_dir = save_dir or os.path.join(_BASE_DIR, "data", "processed")
    ...
    def process_and_save(self):
        ...
        print(f" Cửa sổ trượt đã hoàn tất! window_size = {self.window_size}")
        print(f"X_train shape: {X_train.shape}, y_train shape: {y_train.shape}")
        print(f"X_test  shape: {X_test.shape},  y_test  shape: {y_test.shape}")
        print(f" Dữ liệu đã được lưu vào {self.save_dir}/")


if __name__ == "__main__":
    # Default: read VIC.csv from data/raw/, save to data/processed/
    default_csv = os.path.join(_BASE_DIR, "data", "raw", "VIC.csv")
