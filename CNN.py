...
print("Kết quả đánh giá tập huấn luyện：")
...
train_results.to_csv("train_results/CNN_train_prediction.csv", index=False)
...
plt.title("CNN Training Prediction Performance")
plt.xlabel("Time Step")
plt.ylabel("Close Price")
plt.legend()
plt.tight_layout()
plt.savefig("figures/CNN_train_fit_plot.png")
plt.show()
...
print("Kết quả đánh giá tập kiểm tra：")
...
test_results.to_csv("train_results/CNN_test_prediction.csv", index=False)


plt.figure(figsize=(16, 6))
plt.plot(y_test_inv, label="Actual Test", color="blue")
plt.plot(y_pred_inv, label="Predicted Test", color="red", alpha=0.7)
plt.title("CNN Test Prediction Performance")
plt.xlabel("Time Step")
plt.ylabel("Close Price")
plt.legend()
plt.tight_layout()
plt.savefig("figures/CNN_test_fit_plot.png")
plt.show()
...
evaluation_metrics.to_csv("train_results/CNN_evaluation_metrics.csv", index=False)


model.save("train_results/CNN_model.h5")


np.save("train_results/y_pred_train_CNN.npy", y_train_pred_inv)
np.save("train_results/y_train_CNN.npy", y_train_inv)
np.save("train_results/y_pred_CNN.npy", y_pred_inv)
np.save("train_results/y_test_CNN.npy", y_test_inv)
